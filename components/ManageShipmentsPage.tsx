...
const ManageShipmentsPage: React.FC<ManageShipmentsPageProps> = ({ onNavigate }) => {
  ...
  if (isShipmentDetailsPanel && selectedShipmentForDetails) {
    return (
        <div className="flex flex-col h-full bg-[#161616] animate-fade-in-fast relative">
            {/* Header */}
                ...
                {isActionsDropdownOpen && (
                  <Portal>
                    ...
                    <div
                      ...
                      className="fixed bg-[#262626] border border-[#393939] rounded shadow-xl py-1 animate-drop-down origin-top-right"
                      style={{
            ...
            <div className="flex-1 overflow-y-auto p-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-[#212121] p-4 rounded border border-[#393939]">
                        <div className="text-[12px] text-[#8d8d8d] uppercase mb-1">Status</div>
                        <div className="text-[#f4f4f4] font-medium">{selectedShipmentForDetails.status}</div>
                    </div>
                    <div className="bg-[#212121] p-4 rounded border border-[#393939]">
                        <div className="text-[12px] text-[#8d8d8d] uppercase mb-1">Origin / Destination</div>
                        <div className="text-[#f4f4f4] text-[14px]">{selectedShipmentForDetails.originWarehouse}</div>
                        <div className="text-[#8d8d8d] text-[12px]">to {selectedShipmentForDetails.destination}</div>
                    </div>
                    <div className="bg-[#212121] p-4 rounded border border-[#393939]">
                        <div className="text-[12px] text-[#8d8d8d] uppercase mb-1">Carrier</div>
                        <div className="text-[#f4f4f4]">{selectedShipmentForDetails.carrier}</div>
                    </div>
                </div>

                <h3 className="text-[16px] font-semibold text-[#f4f4f4] mb-4">Shipment Contents</h3>
                <div className="bg-[#212121] border border-[#393939] rounded overflow-hidden">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-[#393939] bg-[#2a2a2a] text-[12px] text-[#8d8d8d] uppercase">
                                <th className="px-4 py-3 font-semibold">SKU</th>
                                <th className="px-4 py-3 font-semibold">Name</th>
                                <th className="px-4 py-3 font-semibold text-right">Quantity</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-[#393939] text-[14px] text-[#c6c6c6]">
                            {selectedShipmentForDetails.items && selectedShipmentForDetails.items.length > 0 ? (
                                selectedShipmentForDetails.items.map((item, idx) => (
                                    <tr key={idx} className="hover:bg-[#2a2a2a] transition-colors">
                                        <td className="px-4 py-3 font-mono text-[#f4f4f4]">{item.sku}</td>
                                        <td className="px-4 py-3">{item.name || <span className="text-[#525252] italic">No name provided</span>}</td>
                                        <td className="px-4 py-3 text-right font-mono text-[#f4f4f4]">{item.quantity}</td>
                                    </tr>
                                ))
                            ) : (
                                <tr>
                                    <td colSpan={3} className="px-4 py-8 text-center text-[#8d8d8d]">No items in this shipment.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
  ...
  return (
    <div className="flex flex-col h-full relative overflow-hidden">
       ...
       <div className="px-6 pt-6 pb-2 shrink-0 animate-fade-in-fast">
        <h1 className="text-[24px] font-semibold text-[#f4f4f4]">Manage Shipments</h1>
        <p className="text-[14px] text-[#8d8d8d]">View and manage all your shipments.</p>
       </div>
       ...
       <div className="px-6 pb-6 bg-transparent">
        <div className="bg-[#262626] border-x border-b border-[#393939] rounded-b-lg">
