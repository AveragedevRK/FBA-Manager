...
interface ShipmentsToolbarProps {
  ...
  onRefresh: () => void; // New: Refresh function
  isLoading: boolean; // New: Loading state from hook
}
        ...
        <div className="hidden lg:block w-[1px] h-[32px] bg-[#393939]"></div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 lg:pb-0 no-scrollbar animate-fade-in-fast flex-nowrap" style={{ animationDelay: '50ms' }}>
            ...
            <button 
              ...
              onClick={onRefresh}
              disabled={isLoading}
              className={`h-[40px] w-[40px] shrink-0 flex items-center justify-center bg-[#2a2a2a] border border-[#393939] rounded-md hover:bg-[#333] hover:border-[#8d8d8d] text-[#c6c6c6] hover:text-[#f4f4f4] transition-all duration-200 relative
